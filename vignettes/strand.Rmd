...
This data can be provided to the simulator directly using objects stored in memory, or the simulator can read the data from binary files stored in feather format on disk. An example of setting up a simulation using in-memory objects as input can be found in the package documentation examples (see `?Simulation`). In this vignette we'll be using a set of (fake) sample data, in binary file format, available in `strand`'s GitHub repository. We assume that [sample_data.tar.gz](https://github.com/strand-tech/strand/raw/master/sample_data.tar.gz) has been extracted in the `vignettes` directory of the package.
