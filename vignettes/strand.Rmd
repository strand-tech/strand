...
`strand` reads each type of input data from files stored in the language-independent binary `feather` format. In this vignette we'll be using a set of (fake) sample data available in `strand`'s GitHub repository. We assume that [sample_data.tar.gz](https://github.com/strand-tech/strand/raw/master/sample_data.tar.gz) has been extracted in the `vignettes` directory of the package.
...
```{r, echo = FALSE}
overall_stats <- res$overallStatsDf()
```
