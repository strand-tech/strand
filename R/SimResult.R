...
SimResult <- R6Class(
  ...
  public = list(
    ...
    #' @description Write a rendered RMarkdown document of the simulation 
    #' results.
    #' @param res The object of class 'SimResult' which we want to write the 
    #' report about.
    #' @param out_loc Directory in which output files should be created and 
    #' filename.
    #' @param out_fmt Format in which output files should be created. The 
    #' default is html and that is currently the only option.
    writeReport = function(out_loc, out_fmt="html") {
      rmarkdown::render(input = system.file("rmd/simReport.Rmd", 
                                            package = "strand"),
